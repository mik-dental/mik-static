(function () {
  ...
  function initLottie() {
    ...
    if (!window.lottie) {
      return { registry, loadElement: () => null, playbackObserver: null };
    }
    ...
    function loadElement(element) {
      ...
      const initialDirection =
        Number(element.dataset.direction) === -1 ? -1 : 1;
    ...
    // Playback IntersectionObserver to pause/play already instantiated animations
    ...
    // Lazy instantiation observer for below-the-fold animations
    ...
    elements.forEach((element) => {
      const isAutoplayHero =
        parseBoolean(element.dataset.autoplay) &&
        (element.closest(".pros-hero-section") ||
          element.classList.contains("lottie-animation-5") ||
          element.classList.contains("hero_lottie"));

      const isInsideHiddenTab = element.closest(".w-tab-pane:not(.w--tab-active)");

      if (isAutoplayHero && !isInsideHiddenTab) {
        const record = loadElement(element);
    ...
    // Interaction gating: arm the lazyObserver only after the first user interaction
    let armed = false;
    function armObserver() {
      ...
      window.removeEventListener("scroll", armObserver, { passive: true });
      window.removeEventListener("wheel", armObserver, { passive: true });
      window.removeEventListener("touchstart", armObserver, { passive: true });
      window.removeEventListener("pointerdown", armObserver, { passive: true });
      window.removeEventListener("keydown", armObserver);
    }

    window.addEventListener("scroll", armObserver, { once: true, passive: true });
    window.addEventListener("wheel", armObserver, { once: true, passive: true });
    window.addEventListener("touchstart", armObserver, { once: true, passive: true });
    window.addEventListener("pointerdown", armObserver, { once: true, passive: true });
    window.addEventListener("keydown", armObserver, { once: true });

    window.addEventListener("resize", () => {
      registry.forEach(({ animation }) => animation.resize());
    });

    return { registry, loadElement, playbackObserver };
  }

  function resizeAndPlayLotties(container, registry, loadElement, playbackObserver) {
    if (!container) {
      return;
    }

    const lottieElements = toArray(
      container.querySelectorAll("[data-animation-type=\"lottie\"][data-src]"),
    );

    lottieElements.forEach((element) => {
      let record = registry.get(element);
      if (!record && typeof loadElement === "function") {
        record = loadElement(element);
        if (record && playbackObserver) {
          playbackObserver.observe(element);
        }
      }

      if (!record || !isVisible(element)) {
        return;
      }
  ...
  function initTabs(lottieContext) {
    const { registry, loadElement, playbackObserver } = lottieContext || {};
    ...
    tabsList.forEach((tabs) => {
      ...
      const activateTab = (
        ...
        if (activePane && shouldLoadLotties) {
          resizeAndPlayLotties(activePane, registry, loadElement, playbackObserver);
        }
      ...
      activateTab(activeLink, false, false);
    });
  }
  ...
  onReady(() => {
    ...
    const lottieContext = initLottie();
    initTabs(lottieContext);
    initSlider();
    initFaq();
